...
- Prints a client-side prompt with a single `frp-client-bootstrap.sh` command. That command downloads the local helper into the target project, writes the FRP client config, installs lightweight `frpc` if missing, and runs `doctor`.
